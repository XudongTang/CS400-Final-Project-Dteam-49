...
public class Main extends Application implements EventHandler<ActionEvent> {
	...
	public void start(Stage primaryStage) {
		try {
				...
				public void handle(ActionEvent e) {
					...
					upload2.setOnAction(new EventHandler<ActionEvent>() {
						public void handle(ActionEvent x) {
							...
							final ComboBox combo1 = new ComboBox(nutrient);
							final ComboBox combo2 = new ComboBox(comparator);
			...
			root.getChildren().addAll(list1, list2, label1, label2, label3, btn1, btn2, btn3, btn4, btn5, btn6, btn7,
					btn8);
	...
	private List<FoodItem> sort(java.util.List<FoodItem> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = 0; j < sortedList.size(); j++) {
				if (curFood.getName().toLowerCase().compareTo(sortedList.get(j).getName().toLowerCase()) < 0) {
